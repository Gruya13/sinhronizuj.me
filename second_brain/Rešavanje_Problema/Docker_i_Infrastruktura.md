# Docker i Infrastruktura (DevOps)
...
*   **Redis Perzistencija**: Redis se pokreće sa `--appendonly yes` flegom (AOF) i montiranim perzistentnim volumenom `redis_data` kako se Celery poslovi ne bi izgubili. Vidi [[Backend_Dokumentacija]].
...
*   **Kontejnerizovan Frontend**: Klijentska React/Vite aplikacija se servira kroz laganu Nginx instancu unutar Docker kontejnera na portu `3000`. Vidi [[Frontend_Dokumentacija]].
...
4.  **Restore drill (Oporavak)**: Uputstvo za oporavak u slučaju katastrofe nalaze se u [backup_restore_uputstvo.md](file:///home/gruya/Projektri/sinhronizuj.me/backup_restore_uputstvo.md). Oporavak baze i S3 skladišta se vrši pomoću skripti `scripts/restore.sh` i `scripts/restore_s3.py`. Vidi više u [[Baza_Podataka]].
...
*   **Koraci**: Podizanje Python okruženja, pokretanje PostgreSQL servisnog kontejnera u GitHub Actions-u, instalacija zavisnosti, Ruff linting, i pokretanje testova preko `pytest`. Testovi se izvršavaju nad realnom Postgres test bazom.
...
*   **CD Logika (Hetzner VPS)**:
    *   Gradi Docker slike za backend (FastAPI) i frontend (React + Nginx).
    *   Radi push slika na **GitHub Container Registry (GHCR)**.
    *   Povezuje se preko SSH na VPS.
    *   Radi `docker compose pull` da povuče slike.
    *   Radi `docker compose up -d` za brzu i bezbednu zamenu aktivnih kontejnera.
...
*   **Koraci**: Automatski generiše oznaku verzije (tag na osnovu trenutnog datuma i vremena, npr. `v2026.06.12-0926`) i kreira GitHub Release.
