...
1. **Control Plane (Hetzner VPS)**: Hostuje klijentsku aplikaciju (React), modularni API server (FastAPI), bazu podataka (PostgreSQL), keš/broker sloj (Redis sa AOF perzistencijom), S3 kompatibilno skladište podataka (MinIO/R2/Hetzner S3) i Celery radnike za orkestraciju i lokalnu obradu audio/video signala.
2. **Compute Plane (Modal.com)**: Serverless platforma na kojoj su raspoređeni GPU kontejneri koji se pokreću po potrebi (Scale-to-Zero) za teške mašinske proračune (vokalna separacija, transkripcija, prevođenje, lektura, glasovna sinteza sa kloniranjem i vizuelni lipsync).
...
flowchart TB
    ...
    subgraph Control Plane (Hetzner VPS - Docker Compose)
        ...
        DB[(PostgreSQL)]
        Cache[(Redis - Broker / Rate Limit / Drafts / Blocklist)]
        S3[(S3 Object Storage - MinIO/R2/Hetzner)]
    ...
    subgraph Compute Plane (Modal.com - Serverless GPU-ovi)
        ...
        ASR[Faster-Whisper + SenseVoiceSmall - NVIDIA T4]
        ...
        Wav2Lip[Wav2Lip LipSync - NVIDIA T4]
    end
    ...
    Celery <-->|Orkestracija i AI Pozivi / Async Polling| Demucs
    Celery <-->|Orkestracija i AI Pozivi / Async Polling| ASR
    Celery <-->|Orkestracija i AI Pozivi / Async Polling| QwenVL
    Celery <-->|Orkestracija i AI Pozivi / Async Polling| QwenLektor
    Celery <-->|Orkestracija i AI Pozivi / Async Polling| TTS
    Celery <-->|Orkestracija i AI Pozivi / Async Polling| Wav2Lip
...
*   **LandingPage ([LandingPage.jsx](file:///home/gruya/Projektri/sinhronizuj.me/frontend/src/components/Landing/LandingPage.jsx))**: Uvodna prezentaciona stranica za neprijavljene korisnike koja ih upoznaje sa platformom i preusmerava na prijavu.
...
4.  **Transkripcija i Vremensko Pozicioniranje (STT)**:
    *   Fajl `vocals.wav` se šalje na Modal ASR radnike (Faster-Whisper na [stt_worker.py](file:///home/gruya/Projektri/sinhronizuj.me/modal_workers/stt_worker.py) i SenseVoiceSmall na [sensevoice_worker.py](file:///home/gruya/Projektri/sinhronizuj.me/modal_workers/sensevoice_worker.py)).
    *   **Faster-Whisper** vrši transkripciju i kreira precizne vremenske odrednice na nivou reči (word-level timestamps).
    *   **SenseVoice-Small** vrši paralelnu transkripciju celog vokala bez vremenskih oznaka, a zatim se vrši LLM arbitraža za ispravku eventualnih grešaka.
...
3.  **Dynamic Time Stretching (Vremensko Rastezanje)**:
    ...
    *   Sistem računa odnos trajanja i pomoću FFmpeg-a **usporava video frejmove i pozadinsku muziku** na tom specifičnom vremenskom intervalu (maksimalno do 1.05x) kako bi se video prilagodio govoru.
...
5.  **LipSync Sinhronizacija (Wav2Lip)**:
    *   Sistem proverava učešće lica u video fajlu. Ako se lice detektuje u više od 10% frejmova, video i spojen srpski vokal se šalju na serverless Wav2Lip modul na Modalu ([modal_workers/wav2lip_worker.py](file:///home/gruya/Projektri/sinhronizuj.me/modal_workers/wav2lip_worker.py)) koji koristi NVIDIA T4 GPU.
    *   **Wav2Lip** model na Modalu vrši fotorealističnu modifikaciju usana govornika kako bi se pokret usana savršeno poklapao sa izgovorenim srpskim rečima, što drastično ubrzava proces i rasterećuje VPS resurse.
6.  **Konačno Spajanje i Upload**:
    *   FFmpeg spaja finalni video sa miksovanim audiom.
    *   Fajl se postavlja na S3 kompatibilno skladište pod ključem `projects/{project_id}/final_video.mp4`, a status projekta u bazi podataka se menja u `COMPLETED`. Klijent preko polling-a detektuje uspešan završetak i nudi video za reprodukciju i preuzimanje.
