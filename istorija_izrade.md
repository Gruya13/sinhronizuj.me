...
- **Opis:**
  ...
  3. **Ispravka CLI alata i usklađivanje admin lozinke:** Identifikovano je da CLI alat [backend/cli.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/cli.py) nije ažurirao lozinku postojećem korisniku već ga je samo promovisao u admina. Skripta je izmenjena tako da uvek ažurira `password_hash` na novu vrednost. Nakon toga, uspešno je izvršena CLI komanda na produkciji i lozinka administratora `grujovic.igor89@gmail.com` je uspešno resetovana na tačnu vrednost `E89120d9dfeb#` (sa malim `b` na kraju).
  4. **Rešavanje CORS problema (Failed to fetch):** Uočen je problem sa CORS politikom gde je preflight OPTIONS zahtev na `/api/v1/auth/login` bio blokiran jer produkcioni domen `https://sinhronizuj.me` nije bio u listi dozvoljenih domena (`ALLOWED_ORIGINS`). U [backend/core/config.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/core/config.py) smo dodali produkcione domene u podrazumevane vrednosti, a takođe smo ažurirali `.env` na produkciji i restartovali API kontejner.
  5. **Promena naslova u kartici pretraživača:** Izmenjen je naslov stranice u [frontend/index.html](file:///home/gruya/Projektri/sinhronizuj.me/frontend/index.html) sa podrazumevanog `frontend` na `Sinhronizuj.me` radi profesionalnijeg vizuelnog identiteta aplikacije. S obzirom na to da spoljni Nginx na VPS-u servira statičke fajlove direktno sa hosta (`/opt/sinhronizuj-me/frontend/dist`), prekopirao sam izgrađeni `dist` sadržaj iz rekreiranog `sinhronizuj-frontend` kontejnera na host VPS-a kako bi promena postala javno vidljiva.
  6. **Ispravka 422 greške pri uploadu videa:** Rešen je problem gde je upload videa vraćao `422 Unprocessable Entity` na ruti `/api/v1/storage/upload_url`. Bezbednosno ojačavanje je uvelo obavezan parametar `project_id` na backendu radi izolacije skladišta, koji frontend nije prosleđivao. Ažurirao sam API klijent [frontend/src/services/api.js](file:///home/gruya/Projektri/sinhronizuj.me/frontend/src/services/api.js) i uploader u [frontend/src/context/StudioContext.jsx](file:///home/gruya/Projektri/sinhronizuj.me/frontend/src/context/StudioContext.jsx) da ispravno prosleđuju `currentProjectId` prilikom traženja presigned upload URL-a.
  7. **Usklađivanje S3/MinIO kredencijala (InvalidAccessKeyId):** Uočena je greška `InvalidAccessKeyId` prilikom pozivanja operacije `CreateBucket`. MinIO se podizao sa podrazumevanim kredencijalima `minioadmin`, dok je API server koristio ojačane kredencijale iz `.env`. Uskladio sam kredencijale tako što sam dodao `MINIO_ROOT_USER` i `MINIO_ROOT_PASSWORD` u `.env` na produkciji i rekreirao MinIO i API kontejnere, čime je autentifikacija na skladištu sada u potpunosti ispravna.
- **Status:** Završeno. CI/CD blokade, CORS problemi, vizuelne izmene naslova, uploader greške i S3 autentifikacija na skladištu su u potpunosti integrisani, testirani i uspešno deploy-ovani na produkciji.
...
- **Opis:**
  ...
  1. **Postgres u CI testovima:** Konfigurisani su testovi u [tests/conftest.py](file:///home/gruya/Projektri/sinhronizuj.me/tests/conftest.py) da koriste PostgreSQL test bazu umesto SQLite-a kako bi se verifikovale UUID kolone i JSON tipovi podataka. Takođe je u [.github/workflows/backend-ci.yml](file:///home/gruya/Projektri/sinhronizuj.me/.github/workflows/backend-ci.yml) dodat PostgreSQL servis.
  2. **Golden testovi za merger.py:** Napisani su detaljni testovi u [tests/test_merger.py](file:///home/gruya/Projektri/sinhronizuj.me/tests/test_merger.py) za pokrivanje kolizije segmenata, ducking efekta i A/V sinhronizacije.
  3. **Refaktorisana struktura main.py:** Monolitna datoteka [backend/main.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/main.py) je podeljena na zasebne modularne delove:
     - [backend/core/schemas.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/core/schemas.py) (Pydantic modeli)
     - [backend/core/limiter.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/core/limiter.py) (Limiter stopa)
     - [backend/services/s3.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/services/s3.py) i [backend/services/redis.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/services/redis.py) (pomoćni servisi)
     - [backend/routes/auth.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/routes/auth.py), [backend/routes/projects.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/routes/projects.py), [backend/routes/segments.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/routes/segments.py), [backend/routes/admin.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/routes/admin.py) i [backend/routes/system.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/routes/system.py) (FastAPI APIRouter-i).
  4. **Popravljanje poznatih bagova:** Popravljen je bag u [backend/routes/segments.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/routes/segments.py) gde se pri overlay-u u generisanju pojedinačnog TTS segmenta vreme pregažene dužine iz baze pogrešno računalo (novim umesto starim trajanjem segmenta), što je dovodilo do desinhronizacije zvuka.

## [2026-06-20 07:00:00] Prioritet P2: Skalabilnost i optimizacija troškova (Scalability & Cost Hardening)
- **Opis:**
  Uspešno smo implementirali i verifikovali sve aspekte prioriteta P2 na grani `hardening/p2-scalability-quality`:
  1. **Migracija Wav2Lip na Modal:** Prebacili smo tešku Wav2Lip inferenciju sa lokalnog VPS-a na serverless Modal GPU worker ([modal_workers/wav2lip_worker.py](file:///home/gruya/Projektri/sinhronizuj.me/modal_workers/wav2lip_worker.py)) koji koristi NVIDIA T4 GPU i podržava preuzimanje check-pointa na NFS deljeno skladište (`sinhronizuj-models-nfs`).
  2. **Asinhrone Modal pozivnice:** Unapredili smo klijentsku logiku u [backend/worker/utils.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/worker/utils.py) da se dugotrajni Modal pozivi (poput prevođenja, transkripcije, sinteze, lipsync) pozivaju asinhrono (polling model preko `/submit` i `/status/{job_id}`) sa ugrađenim automatskim fallback-om na sinhroni režim za unazadnu kompatibilnost. Integrisali smo Modal u [backend/worker/lipsync.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/worker/lipsync.py) i za puni i za selektivni LipSync.
  3. **Migracija storage-a (Hetzner S3 i Cloudflare R2):** Omogućili smo laku konfiguraciju za prelazak sa lokalnog MinIO-a na bilo koji S3 kompatibilni provajder (Hetzner S3, Cloudflare R2 ili AWS) kroz `STORAGE_PROVIDER` i `S3_` konfiguracione parametre u [backend/core/config.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/core/config.py) i zamenili hardkodovane us-east-1 regione u S3 klijentima sa dinamičkim `settings.S3_REGION`.
  4. **Kvote i limiti:** Implementirali smo bezbednosne provere i limite na nivou API-ja u ruti `/api/v1/process-video` u [backend/routes/projects.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/routes/projects.py). Ograničili smo maksimalnu veličinu pojedinačnog fajla (`MAX_SINGLE_FILE_SIZE_MB`), ukupnu dnevnu veličinu upload-a (`MAX_DAILY_UPLOAD_MB`) i ukupno dnevno trajanje obrade videa (`MAX_DAILY_DURATION_SEC`) po korisniku, koristeći Redis za praćenje i ffprobe za brzo čitanje metapodataka o trajanju.
  5. **Kalkulator troškova:** Integrisali smo precizni blended cost model na osnovu stvarnog GPU vremena i resursa. Ažurirali smo `add_phase_cost` u [backend/worker/tasks.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/worker/tasks.py) da se za LipSync na Modalu automatski računa stvarna stopa od `$0.00018 / s` za T4 GPU, dok lokalni fallback ima cenu `$0.0 / s`.
...
### 05.06.2026. 14:48 — Kreiranje kompletne tehničke dokumentacije projekta u folderu `doc`
- **Zahtevi:** Korisnik je zatražio da se pored postojećeg tajnog foldera `sicret doc` kreira novi folder `doc` u kojem će biti urađena potpuna tehnička dokumentacija svih aspekata projekta (uvod, backend, frontend, infrastruktura, modal workers, testiranje i glosar). Izmene moraju biti vidljive za git (verzionisane).
- **Urađeno:**
    * **Konfiguracija (.gitignore):** Modifikovana datoteka [.gitignore](file:///home/gruya/Projektri/sinhronizuj.me/.gitignore) u korenu projekta da eksplicitno isključi folder `doc/` i sve `.md` fajlove u njemu iz ignorisanja (`!doc/**/*.md` i `!doc/*.md`), čime je omogućeno njihovo praćenje na GitHub repozitorijumu.
    * **Dokumentacija (doc/):** Kreiran je novi folder `doc/` koji sadrži sledeće ažurirane tehničke fajlove na srpskom jeziku:
        1. [uvod.md](file:///home/gruya/Projektri/sinhronizuj.me/doc/uvod.md): Svrha platforme i hibridna cloud arhitektura.
        2. [backend.md](file:///home/gruya/Projektri/sinhronizuj.me/doc/backend.md): FastAPI rutiranje, JWT, modeli baze, Celery orkestracija, izolacija radnog prostora i paralelna ekstrakcija.
        3. [frontend.md](file:///home/gruya/Projektri/sinhronizuj.me/doc/frontend.md): React SPA, novi kompaktni DAW layout bez skrolovanja, integracija miksera u plejer, zumiranje, horizontalno skrolovanje, i preklapanja tooltipova na vremenskoj liniji.
        4. [infrastruktura.md](file:///home/gruya/Projektri/sinhronizuj.me/doc/infrastruktura.md): Docker Compose servisi, MinIO S3 bucket-i, UFW firewall pravila i automatizovani noćni SQL bekap.
        5. [modal_workers.md](file:///home/gruya/Projektri/sinhronizuj.me/doc/modal_workers.md): Serverless GPU modeli na Modal.com, Resemble Enhance integracija na L4 GPU, Piper length_scale i popravka FFmpeg asplit vokalnog toka.
        6. [objasnjenje_glosara.md](file:///home/gruya/Projektri/sinhronizuj.me/doc/objasnjenje_glosara.md): Dinamički glosar i novi Python Regex post-processor za automatsko čišćenje prevoda (deklinacija "Ej Aj", jednina robotike, ti-obraćanje i leksičke popravke).
        7. [testiranje.md](file:///home/gruya/Projektri/sinhronizuj.me/doc/testiranje.md): Korak po korak scenariji za ručni test studija, API ruta, i smernice za pytest/vitest/Playwright.
    * **Verifikacija:** Izvršena komanda `git status` koja potvrđuje da git uspešno detektuje direktorijum `doc/` kao untracked fajlove koji se mogu dodati u repozitorijum.
- **Status:** Završeno. Nova i kompletna tehnička dokumentacija je u potpunosti kreirana i verzionisana.

### 05.06.2026. 14:52 — Prestanak praćenja osetljivih markdown dokumenata na GitHub-u
- **Zahtevi:** Nakon potvrde uspešne izrade dokumentacije u folderu `doc`, korisnik je zatražio da se istorija izrade, opis toka podataka i arhitektura uklone sa GitHub repozitorijuma (tj. prestanu pratiti u Git-u ali da ostanu lokalno na disku).
- **Urađeno:**
    * **Konfiguracija (.gitignore):** Izbrisana pravila izuzetaka (`!istorija_izrade.md`, `!pipeline_tok.md`, `!arhitektura_i_tok_podataka.md`) iz [.gitignore](file:///home/gruya/Projektri/sinhronizuj.me/.gitignore), čime su ovi fajlovi vraćeni pod opšte pravilo ignorisanja svih `.md` datoteka.
    * **Git indeks:** Pokrenuta komanda `git rm --cached` za datoteke `istorija_izrade.md`, `pipeline_tok.md` i `arhitektura_i_tok_podataka.md`. Time su fajlovi uspešno uklonjeni iz Git praćenja (ali su sačuvani na lokalnom disku).
- **Status:** Završeno. Fajlovi su uklonjeni sa Git indeksa i više se neće nalaziti na GitHub repozitorijumu.

### 05.06.2026. 14:54 — Prestanak praćenja objašnjenja glosara u doc direktorijumu na GitHub-u
- **Zahtevi:** Korisnik je zatražio da se datoteka `doc/objasnjenje_glosara.md` takođe skloni sa GitHub-a (prestane pratiti ali ostane lokalno).
- **Urađeno:**
    * **Konfiguracija (.gitignore):** Dodato pravilo `doc/objasnjenje_glosara.md` u [.gitignore](file:///home/gruya/Projektri/sinhronizuj.me/.gitignore) na kraj markdown pravila.
    * **Git indeks:** Pokrenuta komanda `git rm --cached doc/objasnjenje_glosara.md` koja uklanja ovaj fajl iz praćenja (ali ga čuva lokalno na disku).
- **Status:** Završeno. Fajl je uklonjen iz Git indeksa i biće sklonjen sa GitHub-a nakon sledećeg push-a.

### 05.06.2026. 14:56 — Prestanak praćenja audit izveštaja na GitHub-u i ažuriranje README-a
- **Zahtevi:** Korisnik je zatražio da se datoteka `audit_izvestaj.md` prestane pratiti na GitHub-u (ukloni iz git praćenja ali ostane lokalno) i da se ažurira `README.md` sa trenutnim stanjem projekta.
- **Urađeno:**
    * **Konfiguracija (.gitignore):** Izbrisano pravilo izuzetka (`!audit_izvestaj.md`) iz [.gitignore](file:///home/gruya/Projektri/sinhronizuj.me/.gitignore), čime je ovaj fajl vraćen pod opšte pravilo ignorisanja svih `.md` datoteka.
    * **Git indeks:** Pokrenuta komanda `git rm --cached audit_izvestaj.md` koja uklanja fajl iz praćenja (ali ga čuva lokalno na disku).
    * **README.md:** Kompletno ažuriran [README.md](file:///home/gruya/Projektri/sinhronizuj.me/README.md) kako bi reflektovao trenutno stanje projekta: Resemble Enhance i OpenVoice V2 integraciju, vLLM servere na Modalu, niže troškove Lektora na A10G, novi kompaktni DAW layout Studija bez skrolovanja sa plejer slajderima, stabilni FFmpeg asplit, Regex post-processor i status završenog rada na glosaru.
- **Status:** Završeno. Audit izveštaj je uklonjen iz Git praćenja, a README.md je uspešno ažuriran i usaglašen sa sistemom.

### 08.06.2026. 11:05 — Implementacija Landing stranice i Liste Čekanja (Waitlist) za Zatvorenu Betu
- **Zahtevi:** Korisnik je zatražio da se kreira Landing stranica za projekat koja će imati premium Glassmorphism vizuelni stil u tamnom režimu (dark mode), interaktivni demo audio mikser na klijentskoj strani za upoređivanje originalnog vokala i srpskog dubbing-a, kao i formu za prijavu na listu čekanja (waitlist) za zatvorenu betu (obzirom da je ovo zatvoreni SaaS).
- **Urađeno:**
    * **Baza podataka (SQLAlchemy):** Kreirana je nova SQLAlchemy klasa `Waitlist` u [models.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/core/models.py) sa kolonama `id`, `email`, `created_at` i `status`.
    * **FastAPI Backend:** Dodat je Pydantic model `WaitlistRequest` i FastAPI endpoint `POST /api/v1/waitlist` u [main.py](file:///home/gruya/Projektri/sinhronizuj.me/backend/main.py). Endpoint normalizuje i validira email formate, sprečava duple prijave na waitlist i registrovane naloge, i upisuje zapise u PostgreSQL bazu sa statusom `"pending"`. Trigerovan je rate limit od 5 zahteva po minuti kako bi se sprečio spam.
    * **Demo Audio:** Generisana su dva testna audio zapisa `demo_en.mp3` i `demo_sr.mp3` u folderu `frontend/public/demo/` radi stabilnog i offline rada interaktivnog miksera na klijentu.
    * **Login/Register:** Modifikovana komponenta [LoginRegister.jsx](file:///home/gruya/Projektri/sinhronizuj.me/frontend/src/components/Auth/LoginRegister.jsx) koja sada prihvata prop `onBack` i prikazuje diskretno dugme za povratak na Landing stranicu.
    * **Landing Page:** Izrađena je potpuno nova, premium React komponenta [LandingPage.jsx](file:///home/gruya/Projektri/sinhronizuj.me/frontend/src/components/Landing/LandingPage.jsx) sa Hero sekcijom, dve forme za waitlist, animiranim waveform talasima i interaktivnim mikserom za ugrađeni engleski original i srpski AI dub koji se u realnom vremenu sinhronizuju na klijentu.
    * **App.jsx:** Integrisana je `LandingPage` u [App.jsx](file:///home/gruya/Projektri/sinhronizuj.me/frontend/src/App.jsx) i uvedeno stanje `showLogin` tako da se svi neprijavljeni posetioci preusmeravaju na Landing umesto na login formu, dok prijavljeni korisnici odmah idu na Dashboard.
    * **index.css:** Ažurirani globalni stilovi u [index.css](file:///home/gruya/Projektri/sinhronizuj.me/frontend/src/index.css) sa responzivnim grid rasporedima, glassmorphism hover efektima i prelepom stilizacijom range slajdera.
    * **Verifikacija:** Izvršena curl testiranja na backendu, psql inspekcija baze podataka, kao i lokalni build frontenda (`npm run build`) koji prolazi uspešno.
- **Status:** Završeno. Nova Landing stranica sa listom čekanja je u potpunosti integrisana i spremna.

### 08.06.2026. 15:52 — Planiranje Infrastrukture i Analize Opterećenja za Close Betu
- **Zahtevi:** Korisnik je zatražio osmišljavanje detaljnog plana za kompletnu infrastrukturu i procenu ponašanja sistema pod opterećenjem od 100 korisnika za predstojeću zatvorenu betu (close beta).
- **Urađeno:**
    * **Plan implementacije:** Kreiran je sveobuhvatan [implementation_plan.md](file:///home/gruya/.gemini/antigravity/brain/d4f7ffac-c6b1-4f9a-99ef-33a53362091d/implementation_plan.md) koji obuhvata detaljnu analizu uskih grla u trenutnoj arhitekturi (konkurentnost Celery radnika, I/O i CPU opterećenje na VPS-u usled audio splicing-a, kvote i hladni startovi na Modal.com).
    * **Preporuke za skaliranje:** Predložen je prelazak na jači Hetzner VPS (CCX22/CPX31 sa 4-8 vCPU i 16GB RAM), prekonfigurisanje Celery queue-ova za razdvajanje teških i brzih zadataka, povećanje PostgreSQL connection pool-a i keširanje lokalnih audio fajlova.
    * **Load Testing:** Definisana je strategija load testiranja pomoću Locust-a koja pokriva simulaciju realnih studijskih sesija (hot-patching, Magic Shorten) i pipeline obrade (upload, analiza, render, polling).
- **Status:** Završeno. Plan je kreiran i poslat korisniku na pregled i autorizaciju.

### 08.06.2026. 15:53 — Implementacija Produkcijske Infrastrukture i Locust Testova za Close Betu
- **Zahtevi:** Izvršavanje plana i postavljanje optimizacija za bazu, Celery i load testiranje pod opterećenjem od 100 korisnika.
- **Urađeno:**
    * **Baza podataka (`backend/core/database.py`):** Povećana propusnost konekcionog pool-a ka PostgreSQL bazi: `pool_size` na 25 i `max_overflow` na 50.
    * **Celery Rutiranje (`backend/worker/celery_app.py`):** Konfigurisana automatska raspodela zadataka po namenskim queue-ovima (`analyze_queue`, `render_queue`, `default_queue`) u zavisnosti od samog tipa zadatka.
    * **Produkciona Konfiguracija (`infra/hetzner/docker-compose.prod.yml`):** Izgrađen Docker Compose za produkciju sa razdvojenim Celery servisima (`sinhronizuj-worker-analyzer`, `sinhronizuj-worker-renderer`, `sinhronizuj-worker-default`) radi paralelne i neometane obrade u piku.
    * **Load Testing (`tests/load_testing/locustfile.py`):** Izrađena Locust skripta za testiranje opterećenja sa simulacijom kompletnog toka klijenta (prijava, listanje, kreiranje projekta, rad u DAW-u sa segmentima, Magic Shorten i hot-patching preview-om).
    * **Verifikacija:** Izvršena py_compile provera za sve modifikovane Python fajlove, što potvrđuje odsustvo sintaksnih grešaka.
- **Status:** Završeno. Sve komponente i konfiguracije su kreirane i uspešno testirane.

### 08.06.2026. 16:00 — Optimizacija Produkcijskog Staka za Hetzner CPX22 Server
- **Zahtevi:** Prilagođavanje konfiguracije novom Hetzner CPX22 serveru (3 vCPU, 4GB RAM) koji je korisnik kreirao, radi prevencije preopterećenja memorije (OOM).
- **Urađeno:**
    * **Konfiguracija (`infra/hetzner/docker-compose.prod.yml`):** Smanjen parametar `--concurrency` sa 2 na 1 za `worker-analyzer` i `worker-renderer` servise. Time se sprečava istovremeno pokretanje više memorijski zahtevnih FFmpeg i pydub procesa na VPS-u, dok CPU operacije ostaju bezbedno raspoređene. `worker-default` je zadržan na `--concurrency=2` za brze operacije preview-a i čišćenja.
- **Status:** Završeno. Konfiguracija je optimizovana i potpuno bezbedna za pokretanje na CPX22 sa 4GB RAM-a.

### 08.06.2026. 16:21 — Kompletno Podešavanje i Pokretanje Novog Hetzner CPX22 Servera (116.202.103.35)
- **Zahtevi:** Povezivanje, inicijalno podešavanje (Swap, Docker, SSH ključevi, Git), prenos `.env` fajla, izrada Nginx obrnutog proksija i pokretanje produkcionog docker compose staka na novom serveru.
- **Urađeno:**
    * **SSH i Ključevi:** Nakon što je korisnik promenio privremenu lozinku na serveru, lokalni SSH javni ključevi su autorizovani u `authorized_keys`. Takođe su preneti privatni i javni ključevi sa lokalne mašine na server kako bi on mogao bezbedno da pristupi privatnim GitHub repozitorijumima.
    * **Swap (4 GB):** Kreiran je i trajno aktiviran Swap fajl od 4 GB u `/swapfile` (upisan u `/etc/fstab`) radi amortizacije memorijskih pikova pod opterećenjem.
    * **Docker i dependencies:** Instalirani su najnoviji Docker Engine, Docker Compose Plugin i Git na operativnom sistemu Ubuntu 26.04 LTS.
    * **Kloniranje i Konfiguracija:** Kloniran je repozitorijum `sinhronizuj.me` na granu `development` u `/opt/sinhronizuj.me` i kreiran je produkcioni `.env` fajl sa optimizovanim MinIO i Redis putanjama koje pokazuju na lokalne kontejnere (`MINIO_ENDPOINT=minio:9000` i javnu IP adresu `http://116.202.103.35:9000` za presigned URL-ove).
    * **Nginx & UFW:** Instaliran je Nginx i kreiran obrnuti proksi koji preusmerava port 80 ka FastAPI serveru na portu 8000. Omogućen je UFW firewall i otvoreni su portovi 22 (SSH), 80 (HTTP), 443 (HTTPS), 9000 i 9001 (MinIO S3).
    * **Pokretanje:** Izgrađene su sve Docker slike iz najnovijeg koda i uspešno je podignut produkcioni docker-compose stak sa razdvojenim Celery queue-ovima na novom serveru.
    * **Verifikacija:** Eksterni curl testovi na `http://116.202.103.35/` su potvrdili da API server uspešno odgovara na zahteve i da je baza podataka u potpunosti funkcionalna.
- **Status:** Završeno. Novi server je 100% podešen, bezbedan, optimizovan i pušten u rad za potrebe close bete.

### 08.06.2026. 21:29 — Sprovođenje Bezbednosnih Optimizacija (Security Hardening) na Serveru
- **Zahtevi:** Povećanje bezbednosti produkcionog servera kroz zabranu lozinki na SSH-u, instalaciju fail2ban-a, certbot-a i smanjenje trajanja presigned URL-ova na 1 sat.
- **Urađeno:**
    * **SSH Hardening (`/etc/ssh/sshd_config`):** Izmenjena konfiguracija SSH servisa i isključeno logovanje lozinkom (`PasswordAuthentication no` i `KbdInteractiveAuthentication no`). Restartovan SSH servis na serveru.
    * **Fail2Ban:** Instaliran i pokrenut `fail2ban` servis za zaštitu od brute-force mrežnih napada.
    * **Certbot:** Instalirani `certbot` i `python3-certbot-nginx` kako bi server bio u potpunosti pripremljen za automatizovano generisanje SSL (HTTPS) sertifikata čim se usmeri domen.
    * **FastAPI Backend (`backend/main.py`):** Smanjeno podrazumevano trajanje MinIO presigned URL-ova sa 24 sata na 1 sat (3600s) u funkciji `get_presigned_download_url`.
