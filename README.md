...
*   **Napredni Prevodilački Pipeline**: Sistem koristi trostepeni proces prevođenja sa spajanjem zavisnih segmenata u logičke rečenice (**Sentence-level Re-segmentation**), višestrukom samokritikom (**Multi-turn Critique** do 3 prolaza) i hibridnim **LLM-as-a-Judge** gatingom (kombinacija CometKiwi modela semantičke sličnosti i detaljne LLM evaluacije) čime se postiže prirodniji tok i gramatika prevoda uz striktno poštovanje ekavice i brojeva ispisanih rečima.
